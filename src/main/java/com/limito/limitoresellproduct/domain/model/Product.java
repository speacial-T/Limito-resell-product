...
public class Product {
	...
	private void setName(String name) {
		if (name == null || name.isBlank()) {
			throw new IllegalArgumentException("상품명은 공백이면 안 됩니다.");
		}
		this.name = name;
	}
	...
	private void setBrandName(String brandName) {
		if (brandName == null || brandName.isBlank()) {
			throw new IllegalArgumentException("브랜드명은 공백이면 안 됩니다.");
		}
	...
	private void setCategoryId(UUID categoryId) {
		if (categoryId == null) {
			throw new IllegalArgumentException("카테고리는 반드시 지정되어야 합니다.");
		}
	...
	private void setOptions(List<Option> options) {
		if (options == null) {
			throw new IllegalArgumentException("옵션은 적어도 하나 있어야 합니다.");
		}
		if (options.size() == 1) {
